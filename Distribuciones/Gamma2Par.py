...
import pylab as pl
import math
import Distribuciones.paramMVGamma2P

def DG2P(gastos, xmedia, sEst):
    if (type(gastos) == str):
        ...
        V_UtMv [:] = -999          # los valores extrapolados para el método de máxima verosimilitud
        ...
        alfaMom = (pow(sEst, 2) / xmedia)  # parametro de escala, método de momentos
        betaMom = pow(xmedia / sEst, 2)    # parámetro de forma, método de momentos
        ...
        for i in range (m):
            ...
            if (matriz01[i, 3] <= 0.5):
                ...
                (1 + (b3 * V_Ut[i, 0]) + (b4 * pow(V_Ut[i, 0], 2)) + (b5 * pow(V_Ut[i, 0], 3))))
                V_Ut[i, 1] = -V_Ut[i, 1]
            matriz01[i, 4] = (alfaMom * betaMom) * ((1 - pow(9 * betaMom, -1) + (V_Ut[i,1] * \
        ...
        parametrosMV = Distribuciones.paramMVGamma2P.parametros(gastos)  # Tupla con alfa y beta por el método de MV
        alfaMv = parametrosMV[0]
        betaMv = parametrosMV[1]
        EE_Mv = 0

        for i in range (m):
            if (matriz01[i, 3] > 0.5):
                V_UtMv[i, 0] = (math.log(1 / pow(1 - matriz01[i, 3], 2))) ** 0.5  # V
                V_UtMv[i, 1] = V_UtMv[i, 0] - ((b0 + (b1 * V_UtMv[i, 0]) + (b2 * pow(V_UtMv[i, 0], 2))) /\
                (1 + (b3 * V_UtMv[i, 0]) + (b4 * pow(V_UtMv[i, 0], 2)) + (b5 * pow(V_UtMv[i, 0], 3))))  # Ut

            if (matriz01[i, 3] <= 0.5):
                V_UtMv[i, 0] = (math.log(1 / pow(matriz01[i, 3], 2))) ** 0.5  # V
                V_UtMv[i, 1] = V_UtMv[i, 0] - ((b0 + (b1 * V_UtMv[i, 0]) + (b2 * pow(V_UtMv[i, 0], 2))) /\
                (1 + (b3 * V_UtMv[i, 0]) + (b4 * pow(V_UtMv[i, 0], 2)) + (b5 * pow(V_UtMv[i, 0], 3))))  # Ut
                V_UtMv[i, 1] = -V_UtMv[i, 1]  # Ut, cambio de signo
            matriz01[i, 5] = (alfaMv * betaMv) * ((1 - pow(9 * betaMv, -1) + (V_UtMv[i,1] * \
            (pow(9 * betaMv, -1)) ** 0.5)) ** 3)              # Se calcula x
            prueba = (matriz01 [i, 1] - matriz01 [i, 5]) ** 2  # Error Estandart momentos
            EE_Mv = EE_Mv + prueba

        EE_Mv = (EE_Mv/(m-2)) ** 0.5  # Se estima el Error Estandart para 2 parametros método MaxVer
        print ("Distribución Gamma 2 parámetros, error estandart método de momentos:", EE_Mom)
        print ("Distribución Gamma 2 parámetros, error estandart método de máxima verosimilitud:", EE_Mv)
        EEstandart[0, 0] = EE_Mom
        EEstandart[0, 1] = EE_Mv

        #********************************************************************************************
        # Se llena la matriz02. Datos extrapolados método momentos 
        #********************************************************************************************
        
        n = 12

        for i in range (n):
            matriz02 [i, 1] = 1 - pow(matriz02[i,0], -1)  # F(x)
            if (matriz02[i, 1] > 0.5):
            ...
            if (matriz02[i, 1] <= 0.5):
                ...
                (1 + (b3 * V_Ut[i, 2]) + (b4 * pow(V_Ut[i, 2], 2)) + (b5 * pow(V_Ut[i, 2], 3))))
                V_Ut[i, 3] = -V_Ut[i, 3]
            matriz02[i, 2] = (alfaMom * betaMom) * ((1 - pow(9 * betaMom, -1) + (V_Ut[i,3] * \
            (pow(9 * betaMom, -1)) ** 0.5)) ** 3)              # Se calcula x
        
        #********************************************************************************************
        # Se llena la matriz02. Datos extrapolados método máxima verosimilitud
        #********************************************************************************************
        ...
        #    matriz02 [i, 1] = 1 - pow(matriz02[i,0], -1)  # F(x)
            ...
            matriz02[i, 3] = (alfaMv * betaMv) * ((1 - pow(9 * betaMv, - 1) + (V_UtMv[i,3] * \
            (pow(9 * betaMv, -1)) ** 0.5)) ** 3)              # Se calcula x
        ...
        titulo0 = "Distribucion Gamma 2 Parámetros (Momentos)\n EE= " + str(EE_Mom)
        titulo1 = "Distribucion Gamma 2 Parámetros (MaxVer)\n EE= " + str(EE_Mv)
        ...
        pl.savefig("salidas/Gamma2P_Momentos.png", dpi=1200)
        pl.show()
        ...
        pl.savefig("salidas/Gamma2PMaxV.png", dpi=1200)
        pl.show()
        ...
        pl.legend(loc="best")
        pl.title("Distribucion Gamma 2 Parametros\n Momentos y Maxima Verosimilitud")
        pl.ylabel("Gastos (m^3/s)")
        pl.semilogx(True)
        pl.grid(True, which='both', color='g', linestyle='-', linewidth=0.5)
        ...
        pl.savefig("salidas/Gamma2PMom_MaxV.png", dpi=1200)
        pl.show()
