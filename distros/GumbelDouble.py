...
def DDG(gastos, estacion):
    ...
    pl.semilogx(True)
    pl.grid(True, which='both', color='g', linestyle='-', linewidth=0.5)
    pl.savefig("salidas/" + estacion + "_DistribucionDGumbelRosen0.png", dpi=1200)
    pl.show()
    ...
    pl.plot(tR, dAjust1, color="r", linewidth="1.0", linestyle="-", label ="Datos Ajustados")
    pl.legend(loc="best")
    pl.title("Distribucion Doble Gumbel (Algoritmo de Rosenbrock): EE= " + str(EE1))
    pl.ylabel("Gastos(m³/s)")
    #pl.xlabel("Periodo de retorno") 
    pl.semilogx(True)
    pl.grid(True, which='both', color='g', linestyle='-', linewidth=0.5)
    ...
    pl.savefig("salidas/" + estacion + "_DistribucionDGumbelRosen1.png", dpi=1200)
    pl.show()
